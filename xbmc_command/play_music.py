...
class Command(core.Command):
  ...
  def __call_artist(self, artist, dry):
    regex = self.__try_regex(artist)
  ...
  def __call_album(self, album, dry):
    regex = self.__try_regex(album)
    sort = {
        'order': 'ascending',
        'ignorearticle': True,
        'method': 'album'
  ...
  def __call_genre(self, genre, dry):
    regex = self.__try_regex(genre)
    sort = {
        'order': 'ascending',
        'ignorearticle': False,
        'method': 'genre'
  ...
  def __get_playlist(self):
    ...
    return filter(lambda pl: pl['type'] == 'audio', playlists['result'])[0]['playlistid']
...
The remaining items will be played by the AudioPlayer.

If the regex matches too many items, the process may tak a while, depending
